...
```
npm install @lime-material-16px/slider
```
...
```js
// CommonJS
const {MDCSlider} = require('@lime-material-16px/slider/dist/mdc.slider');

// AMD
require(['/path/to/@lime-material-16px/slider/dist/mdc.slider'], ({MDCSlider}) => {
  // Use MDCSlider
});
...
The `@lime-material-16px/slider` package ships with an `MDCSliderFoundation` class that framework authors can
