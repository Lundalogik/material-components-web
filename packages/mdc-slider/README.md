...
```js
// CommonJS
const {MDCSlider} = require('@limetech/mdc-slider/dist/mdc.slider');

// AMD
require(['/path/to/@limetech/mdc-slider/dist/mdc.slider'], ({MDCSlider}) => {
  // Use MDCSlider
});
...
The `@limetech/mdc-slider` package ships with an `MDCSliderFoundation` class that framework authors can
