...
```
npm install @lime-material/slider
```
...
```js
// CommonJS
const {MDCSlider} = require('@lime-material/slider/dist/mdc.slider');

// AMD
require(['/path/to/@lime-material/slider/dist/mdc.slider'], ({MDCSlider}) => {
  // Use MDCSlider
});
...
The `@lime-material/slider` package ships with an `MDCSliderFoundation` class that framework authors can
