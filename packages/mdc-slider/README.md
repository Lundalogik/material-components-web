...
```
npm install @limetech/mdc-p2-slider
```
...
```js
import {MDCSlider} from '@limetech/mdc-p2-slider';
...
```js
// CommonJS
const {MDCSlider} = require('@limetech/mdc-p2-slider/dist/mdc.slider');

// AMD
require(['/path/to/@limetech/mdc-p2-slider/dist/mdc.slider'], ({MDCSlider}) => {
  // Use MDCSlider
});
...
The `@limetech/mdc-p2-slider` package ships with an `MDCSliderFoundation` class that framework authors can
